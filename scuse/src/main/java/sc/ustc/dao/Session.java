...
public class Session extends Conversation {
    ...
    public Object load(Class clazz, Integer userId) throws Exception {
        ...
        sql = MessageFormat.format("select {0} from {1} where {2}={3}", columns, mapper.get("table"), ((IdBean)mapper.get("id")).getColumn(), userId);
        String lazySql = MessageFormat.format("from {0} where {1}={2}", mapper.get("table"), ((IdBean)mapper.get("id")).getColumn(), userId);
    ...
    public Object get(Class clazz, Integer userId) throws Exception {
        mapper = mappers.get(clazz.getName());
        sql = MessageFormat.format("select * from {0} where {1}={2}", mapper.get("table"), ((IdBean)mapper.get("id")).getColumn(), userId);
        System.out.println(sql);
...
//            sql = "delete from user where userid=?";
            sql = MessageFormat.format("delete from {0} where {1}={2}", mapper.get("table"), ((IdBean)mapper.get("id")).getColumn(), userId);
            System.out.println(sql);
            MyQueryRunner.update(sql);
            return true;
        }catch (Exception e){
...
//            拼接属性
        ...
        if (sqlType.equals("insert")){
            ...
            return MessageFormat.format("insert into {0}({1}) value({2})", mapper.get("table"), tableInfo, valueInfo);
        }else {
            updateSqlSet.deleteCharAt(updateSqlSet.length() - 1);
            return MessageFormat.format("update {0} set {1} where {2}", mapper.get("table"), updateSqlSet, updateSqlConstraint);
        }
    }
